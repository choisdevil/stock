...
# 여기부터 사용자 입력 후 데이터 불러오는 부분 등 동일하게 유지
...
if ticker_input:
    try:
        ...
        if hist.empty:
            ...
            for line in tech_analysis_results[:-1]:  # 종합 점수 문구는 제외하고 출력
                st.write(line)

            max_score = 6
            st.progress(tech_score / max_score)
            st.metric(label="종합 기술적 조건 점수", value=f"{tech_score} / {max_score}")
